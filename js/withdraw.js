$(function() {
    ...
    socket.on('response', function(info) {
        if (info.data == "balance") $('#useridentification').html("| Balance : <div class='useridentificationCLASS' id='trade'>" + info.value + "</div>");
            ...
            if (info.who == "AF") {
                ...
                document.getElementById('Proccess').style.display = "none";
                $('#modalSucc').html("Your trade offer was proccesed and confirmed .You can go to your trade by clicking <a href=https://steamcommunity.com/tradeoffer/" + info.value+ ">Here</a> Thank you for withdrawing from CsgoWin.gg ! Have a nice day !");
...
function loadbotinv() {
    ...
    $.ajax({
        ...
        success: function(e) {
            "undefined" != typeof e.error ? $("#user").html("<div class='innerError'>Unable to load bot items , if this problem continues , please contact one of the admins </div>") : additems(e, 0)
        }
    })
}
...
if ($($(event.target)["0"].offsetParent).is('.item')) var whoisclicked = $(event.target)["0"].offsetParent;
            ...
            loadbotinv();
        } else if ($(event.target).is('#1')) {
            document.getElementsByClassName('botactive')[0].classList.remove('botactive');
            document.getElementById("1").classList.add('botactive');
        } else if ($(event.target).is('#2')) {
            document.getElementsByClassName('botactive')[0].classList.remove('botactive');
            document.getElementById("2").classList.add('botactive');
        } else if ($(event.target).is('#3')) {
            document.getElementsByClassName('botactive')[0].classList.remove('botactive');
            document.getElementById("3").classList.add('botactive');
        } else {
